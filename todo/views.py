    ...
    if request.method == 'GET':
        ...
        if request.POST['password1'] == request.POST['password2']:
            ...
            try:
                ...
                return render (request, 'signup.html', {'form': UserCreationForm(), 'error':'Username already EXISTS. Try something else'})
        else:
            # Password mis-match
            return render (request, 'signup.html', {'form': UserCreationForm(), 'error':'Password Mismatch'})
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:   
            return render (request, 'loginuser.html', {'form': AuthenticationForm(), 'error':'Username Password MIMSMATCH'})
        else:
            login(request, user)
            return redirect('currenttodos')
